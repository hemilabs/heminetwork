...
// Vendored from ethereum-optimism/optimism tag op-contracts/v1.8.0
// (packages/contracts-bedrock/src/L1/OptimismPortal.sol), the portal that
// proves withdrawals against the L2OutputOracle.  It was removed from later
// optimism releases.  e2e/deploy-l2oo.sh upgrades the OptimismPortalProxy
// deployed by op-deployer to it when PROPOSER_MODE=l2oo.
//
// Changes:
//   - imports updated to the current contracts-bedrock layout, the
//     PortalErrors.sol errors are declared in this file
//   - custom gas token support removed from SystemConfig: gasPayingToken()
//     always returns ETH and setGasPayingToken() was removed
...
// Errors, from src/libraries/PortalErrors.sol which no longer exists
...
import { L2OutputOracle as IL2OutputOracle } from "./L2OutputOracle.sol";
...
contract OptimismPortal is Initializable, ResourceMetering, ISemver {
    ...
    function gasPayingToken() internal view returns (address addr_, uint8 decimals_) {
        // custom gas tokens were removed from SystemConfig, ETH is always
        // the gas paying token
        (addr_, decimals_) = (Constants.ETHER, 18);
    }
