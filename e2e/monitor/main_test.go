...
const (
	...
	localnetPrivateKey = "92db14e403b83dfe3df233f83dfa3a0d7096f21ca9b0d6d6b8d88b2b4ec1564e"
)
...
func isolatedDeployTest(t *testing.T)  {
	// ensure node is synced via eth.syncing === false

	// ensure a new block is being created every 12 seconds

	// set balance of private key to a very high value, as it will need it

	// deploy implementations

	// deploy upgrade via hemi-upgrade

	// ensure that blocks are still being created

	// run tests against deploy
}
...
func hvmTipNearBtcTip(t *testing.T, ctx context.Context, l2Client *ethclient.Client, l2ClientNonSequencing *ethclient.Client, privateKey *ecdsa.PrivateKey) {
	...
	auth, err := bind.NewKeyedTransactorWithChainID(privateKey, big.NewInt(743111))
	if err != nil {
		t.Fatal(err)
	}
...
func deployL1TestToken(t *testing.T, ctx context.Context, l1Client *ethclient.Client, privateKey *ecdsa.PrivateKey) common.Address {
	...
	t.Logf("checking the erc20 l1 balance of %s", fromAddress)
...
func bridgeEthL1ToL2(t *testing.T, ctx context.Context, l1Client *ethclient.Client, l2Client *ethclient.Client, privateKey *ecdsa.PrivateKey) {
	...
	var value *big.Int
	prevBalance, err := l2Client.BalanceAt(ctx, receiverAddress, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < retries; i++ {
		...
		auth.Value = big.NewInt(90000000000000000)
		value = auth.Value
	...
	if !testingFork()&& balance.Cmp(value) < 0 {
...
func bridgeEthL2ToL1(t *testing.T, ctx context.Context, l1Client *ethclient.Client, l2Client *ethclient.Client, privateKey *ecdsa.PrivateKey) {
	...
	if !testingFork() && balance.Cmp(big.NewInt(0)) != 1 || mod.Cmp(big.NewInt(0)) != 0 {
...
func bridgeEthL2ToL1Legacy(t *testing.T, ctx context.Context, l1Client *ethclient.Client, l2Client *ethclient.Client, privateKey *ecdsa.PrivateKey) {
	publicKey := privateKey.Public()
	publicKeyECDSA, ok := publicKey.(*ecdsa.PublicKey)
	if !ok {
		t.Fatal("error casting public key to ECDSA")
	...
	num, err := oracle.LatestBlockNumber(&bind.CallOpts{})
	if err != nil {
		panic(err)
	}
...
func bridgeERC20FromL2ToL1Legacy(t *testing.T, ctx context.Context, l1Address common.Address, l2Address common.Address, privateKey *ecdsa.PrivateKey, l1Client *ethclient.Client, l2Client *ethclient.Client) {
	publicKey := privateKey.Public()
	publicKeyECDSA, ok := publicKey.(*ecdsa.PublicKey)
	if !ok {
		t.Fatal("error casting public key to ECDSA")
	...
	for {
		if balance.String() != "999999999999999999999950" {
			t.Logf("unexpected balance: %d", balance)
		} else {
			break
		}
