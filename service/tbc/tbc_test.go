...
// list. Returns:
// *[][80]byte -> Raw headers
// *[]wire.BlockHeader -> Parsed headers
// *[]chainhash.Hash -> Hashes of blocks, which are calculated and verified against expected values
func getHeaderHashesRange(start int, end int, headerStrs []string, hashStrs []string) ([][80]byte, []wire.BlockHeader, []chainhash.Hash, error) {
	if start > end {
	...
	return rawHeaders, parsedHeaders, calculatedHashes, nil
}
...
func TestExternalHeaderModeSimpleIncorrectRemoval(t *testing.T) {
	...
	processToHeight := 9
	rawHeaders, _, hashes, err := getHeaderHashesRange(0, processToHeight-1, simpleChainHeaders[:], simpleChainHashes[:])
	if err != nil {
		t.Error(err)
	}
	...
	for _, badRange := range badRanges {
		...
		rawHeaders, _, _, err := getHeaderHashesRange(start-1, end-1, simpleChainHeaders[:], simpleChainHashes[:])
		...
		if rt != tbcd.RTChainDangling {
			t.Errorf("removing headers from %d to %d when tip is %d should have failed with tbcd.RTChainDangling", start, end, canonicalHeightBefore)
		}
	...
	// STEP 3: Remove blocks 3 through 9 which should work correctly and verify 2 is canonical afterwards
	// Subtract 1 from start and end because the ranges refer to block heights,
	// but the defined simple chain headers/hashes start at block #1
	start := 3
	end := 9
	...
	processToHeight = 8
	rawHeaders, _, hashes, err = getHeaderHashesRange(3-1, processToHeight-1, simpleChainHeaders[:], simpleChainHashes[:])
	if err != nil {
		t.Error(err)
	}
